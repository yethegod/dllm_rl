...
# === YES/NO helpers begin ===
YES_NO_ENUMERATION_RE = re.compile(r"(?i)\b(?:yes\s*/\s*no|no\s*/\s*yes|yes\s+or\s+no|no\s+or\s+yes)\b")

def _strip_punct_and_decor(s: str) -> str:
    s = s.strip()
    s = re.sub(r'^[\'"\(\[\{<>\s\*]+', '', s)
    s = re.sub(r'[\'"\)\]\}>\s\*\.\!\?\:\;,]+$', '', s)
    return s

def _strip_simple_latex_wrappers(s: str) -> str:
    return re.sub(r'\\[a-zA-Z]+{([^}]*)}', r'\1', s)

def normalize_yes_no_token(s: str, strong: bool = False) -> str | None:

    if not s:
        return None
    s0 = _strip_simple_latex_wrappers(s)
    s0 = _strip_punct_and_decor(s0)
    if not s0:
        return None
    if YES_NO_ENUMERATION_RE.search(s0):
        return None
    t = s0.casefold()
    if t == "yes" or t == "no":
        return t
    if t in ("y", "n"):
        return "yes" if (t == "y" and (strong or len(s0) == 1)) else ("no" if (t == "n" and (strong or len(s0) == 1)) else None)
    return None

ANCHOR_RE = re.compile(
    r"(?is)"  # DOTALL + IGNORECASE
    r"(?:final\s+answer|the\s+answer|answer|result|verdict|prediction)\s*"
    r"(?:is|=|:)?\s*"
    r"(?P<ans>[\s\S]{1,40})"
)

def extract_yes_no_from_text(text: str) -> str | None:

    if not text:
        return None

    try:
        boxed = get_answer_str(text)
        tok = normalize_yes_no_token(boxed)
        if tok is not None:
            return tok
    except Exception:
        pass

    candidates = list(ANCHOR_RE.finditer(text))
    if candidates:
        for m in reversed(candidates):
            frag = m.group("ans")
            frag = frag.strip()
            frag = re.split(r"[\.\!\?\;\:\,\n\r]", frag, maxsplit=1)[0]
            tok = normalize_yes_no_token(frag, strong=True)
            if tok is not None:
                return tok

    tok = normalize_yes_no_token(text)
    if tok is not None:
        return tok
...
async def is_latex_equal(str1, str2, executor, math_mode="legacy"):
    # 快速 yes/no 判定（两侧都能归一为 yes/no 时）
    tok1 = normalize_yes_no_token(get_answer_str(str1))
    tok2 = normalize_yes_no_token(get_answer_str(str2))
    ...
    if math_mode == "legacy":
        ...
        # 在 math_verify 模式下也优先处理 yes/no
        tok1_mv = normalize_yes_no_token(get_answer_str(str1))
        tok2_mv = normalize_yes_no_token(get_answer_str(str2))
        if tok1_mv is not None and tok2_mv is not None:
            return tok1_mv == tok2_mv
...
def get_final_answer(output: str) -> str:
    ...
    tok = extract_yes_no_from_text(output)
    if tok is not None:
        return tok
    ...
    for pat in pattern_list:
        ...
        if matches:
            yn = normalize_yes_no_token(get_answer_str(matches[0]), strong=True)
            if yn is not None:
                return yn
