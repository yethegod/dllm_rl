...
class ModelRunner:
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        # Pick a free port to avoid conflicts when multiple processes start concurrently
        import socket, random
        _dist_port = int(os.environ.get("JETENGINE_DIST_PORT", 0))
        if _dist_port == 0:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as _s:
                _s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                _s.bind(("", 0))
                _dist_port = _s.getsockname()[1]
