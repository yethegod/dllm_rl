        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    ...
                    x0 = torch.argmax(logits_with_noise, dim=-1) # 1, l + gen_length + suffix_len 最大概率位置整数索引
                    
                    # Get confidence scores
                    if remasking == 'low_confidence':
                        p = F.softmax(logits.to(torch.float64), dim=-1) # shape (1, l + gen_length + suffix_len, vocab_size)
                        x0_p = torch.squeeze(
                            torch.gather(p, dim=-1, index=torch.unsqueeze(x0, -1)), -1) # 1, l + gen_length + suffix_len represents the confidence of each x0   x0对应的token的概率各自是多少
                    elif remasking == 'random':
                        x0_p = torch.rand((x0.shape[0], x0.shape[1]), device=x0.device)
                    else:
                        raise NotImplementedError(remasking)
                    ...
                    x0 = torch.where(mask_index, x0, x) # shape (1, l + gen_length + suffix_len)    如果是MASK位置，取新结果，否则取老结果
