...
try:
    ...
    _FLASH_RMS_NORM_AVAILABLE = True
except ImportError:
    _FLASH_RMS_NORM_AVAILABLE = False
...
def fused_flex_attention(query, key, value, attention_mask, **kwargs):
    # During SFT training, LlamaFactory passes a standard 2D padding tensor.
    # SDAR diffusion uses full bidirectional attention during training, so drop any
    # non-BlockMask tensor (block masks for SDAR structure are created externally for inference).
...
class SDARRotaryEmbedding(nn.Module):
    def __init__(self, config: SDARConfig, device=None):
        ...
        if self.rope_type in ROPE_INIT_FUNCTIONS:
            ...
            # 'default' was removed in newer transformers; replicate its behaviour
            def _default_rope_init(config, device=None, **kwargs):
                head_dim = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)
                inv_freq = 1.0 / (config.rope_theta ** (
                    torch.arange(0, head_dim, 2, dtype=torch.int64).float().to(device) / head_dim
                ))
                return inv_freq, 1.0
